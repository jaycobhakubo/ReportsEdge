...
select @reporttypeId = Reporttypeid from ReportTypes where TypeName = 'Bingo';
select @reportid = ReportID from Reports where ReportFileName = 'BingoGameVerificationReport.rpt';

if (@reportid is null)
begin
	insert into Reports (IsActive, ReportFileName, ReportTypeID) values (1, 'BingoGameVerificationReport.rpt', @reporttypeId);
	select @reportid = ReportID from Reports where ReportFileName = 'BingoGameVerificationReport.rpt';
end;
...
begin
	insert into ReportLocalizations (ReportID, CultureID, CultureName, ReportDisplayName) values (@reportid, 1033, 'en-US', 'Bingo Game Verification');
end
...
where ReportFileName like 'BingoGameVerificationReport.rpt';
