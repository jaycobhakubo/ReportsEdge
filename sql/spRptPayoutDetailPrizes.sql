...
/****** Object:  StoredProcedure [dbo].[spRptPayoutDetailPrizes]    Script Date: 10/07/2013 17:49:16 ******/
...
-- 2012.04.11 JKN: Fixed issue with accrual increases not being limited
--	by session
-- 2012.06.27: Removed accrual payouts from cash, check, merchandise, credit,
--  other totals.
--2012.07.20: DE10588 Return the prize fees for Progressives
-- ============================================================================
	@OperatorID	AS INT,
...
begin
	...
	WHERE	(@OperatorID IS NULL OR @OperatorID = p.OperatorID) AND
			...
			p.AccrualTransID is null -- Don't include accruals.
    ...
    FROM AccrualTransactions at
		...
		JOIN SessionPlayed as sp on at.SessionPlayedId = sp.SessionPlayedID and sp.IsOverridden = 0 -- JKN include active sessions
    WHERE at.TransactionTypeID in (4, 5, 37)  -- auto and manual accrual increases and voids
		...
		AND (sp.GamingSession = @Session or @Session is null) -- JKN ensure that the session is taken into account
    ...
    and pt.VoidTransID is null -- Added:(DE10268) Karlo Camacho 3.29.12
