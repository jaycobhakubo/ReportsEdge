...
/****** Object:  StoredProcedure [dbo].[spRptPointSummary]    Script Date: 09/18/2015 16:21:12 ******/
...
And RR.OperatorID = @OperatorID -- DE11700
...
SELECT  P.PlayerID, P.FirstName, P.LastName, 
		...
		RR.SaleSuccess,	RR.RegisterReceiptID, RR.OriginalReceiptID, gtTransTotal,gtTransactionTypeID,
		RD.RegisterDetailID, RD.PackageName, RD.PackagePrice, RD.ReceiptLine, RD.Quantity, RD.DiscountAmount, 
		RD.DiscountPtsPerDollar, isnull(RD.TotalPtsEarned, 0) * -1 /* DE12750 */, RD.VoidedregisterReceiptID, isnull(RD.TotalPtsRedeemed, 0) * -1 /* DE12750 */,
		TT.TransactionType, D.DeviceType , gtdPrevious, gtdPost, gtdVoidDate
FROM Player P (nolock)
Join PlayerInformation PIN (nolock) on P.PlayerID = PIN.PlayerID
JOIN RegisterReceipt RR (nolock) ON P.PlayerID = RR.PlayerID
...
And gtOperatorID = @OperatorID -- DE11700
...
where CAST(convert(varchar(10),rr.GamingDate,10) as smalldatetime) >=  cast(CONVERT(VARCHAR(10),@StartDate  ,10) as smalldatetime)
and cast(CONVERT(VARCHAR(10),rr.GamingDate,10) as smalldatetime) <= cast(CONVERT(VARCHAR(10),@EndDate   ,10) as smalldatetime)
