...
select @reportid = ReportID from Reports where ReportFileName = 'ElectronicSalesReport.rpt';

if (@reportid is null)
begin
	insert into Reports (IsActive, ReportFileName, ReportTypeID) values (1, 'ElectronicSalesReport.rpt', @reporttypeId);
	select @reportid = ReportID from Reports where ReportFileName = 'ElectronicSalesReport.rpt';
end;
...
begin
	insert into ReportLocalizations (ReportID, CultureID, CultureName, ReportDisplayName) values (@reportid, 1033, 'en-US', 'Electronic Sales');
end
...
where ReportFileName like 'ElectronicSalesReport.rpt';
