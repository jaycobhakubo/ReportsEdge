...
/****** Object:  StoredProcedure [dbo].[spRptElectronicDeviceHistory]    Script Date: 03/01/2013 10:45:47 ******/
...
---2013.03.01 knc:DE10842-Number of Cards sold per pack is not correct on the Electronic Device History report-FIXED.
-- 20150921(knc): Add coupon sales.
-- =============================================
@OperatorID     as INT,
...
select  bcd.bcdCardNo,bcd.bcdSessionGamesPlayedID, rr.RegisterReceiptID from RegisterReceipt rr
    ...
    AND RR.GamingDate <= CAST(CONVERT(varchar(12),@EndDate, 101) AS smalldatetime) 
     AND RR.OperatorID=@OperatorID 
      and rdi.CardMediaID <> 2  --Added (knc/3.1.2013/DE10842)  
...
select   RR.TransactionNumber,
         ...
         sum(RDI.Price * RD.Quantity * RDI.Qty ) + isnull(cpn.CouponSales, 0) as Price--,cpn.CouponSales ,
         ,ulUnlockDate, 
         ulPackLoginAssignDate,
         RR.SaleSuccess,
         RR.Unitnumber,
...
From   RegisterReceipt RR  
	   ...
	   LEFT JOIN Machine M  ON ((M.MachineID =  ulSoldToMachineID )or (M.MachineID=RR.SoldToMachineID))
		--Join with coupon sales, This sql script is base on dbo.FindCouponSales
		left join (Select TransactionNumber, CouponSales from dbo.FindCouponSalesByTransaction(@OperatorID,@StartDate,@EndDate,@Session)				
					) cpn on cpn.TransactionNumber = rr.TransactionNumber
...
select   RR.TransactionNumber,
         ...
         RR.PackNumber,
         sum(RDI.CardCount * RD.Quantity) as NoOfCards,
         sum(RDI.Price * RD.Quantity * RDI.Qty ) + isnull(cpn.CouponSales, 0) as Price,
         ulUnlockDate, 
         ulPackLoginAssignDate,
         RR.SaleSuccess,
         RR.Unitnumber,
