...
/****** Object:  StoredProcedure [dbo].[spRptInventoryRetired]    Script Date: 09/03/2015 14:05:06 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[spRptInventoryRetired]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[spRptInventoryRetired]
GO
...
CREATE PROCEDURE [dbo].[spRptInventoryRetired]
(	@OperatorID as int,
...
WHERE (pdi.OperatorID = @OperatorID OR @OperatorID IS NULL)
	...
	And ii.iiRetiredDate >= @StartDate
	And ii.iiReceivedDate <= @EndDate
