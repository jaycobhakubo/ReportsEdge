...
select @reportid = ReportID from Reports where ReportFileName = 'SessionSummaryRecap.rpt';

if (@reportid is null)
begin
	insert into Reports (IsActive, ReportFileName, ReportTypeID) values (1, 'SessionSummaryRecap.rpt', @reporttypeId);
	select @reportid = ReportID from Reports where ReportFileName = 'SessionSummaryRecap.rpt';
end;
...
begin
	insert into ReportLocalizations (ReportID, CultureID, CultureName, ReportDisplayName) values (@reportid, 1033, 'en-US', 'Session Summary Recap');
end
...
where ReportFileName like 'SessionSummaryRecap.rpt'; 