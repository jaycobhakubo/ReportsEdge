...
/****** Object:  StoredProcedure [dbo].[spRptPaperAuditByDate]    Script Date: 04/17/2015 08:52:10 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[spRptPaperAuditByDate]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[spRptPaperAuditByDate]
GO
...
-- Description:	<Paper usage history for each serial number ordered by date
				-- Uses the same logic as spRptPaperAuditSeriesUsage>
-- 2015.04.17 DE12438: If zero was entered for a damaged audit number the report would not
...
CREATE PROCEDURE [dbo].[spRptPaperAuditByDate]
...
select
	...
	CurrentCount
from @FinalResults
Order by GamingDate, GamingSession, MasterTransID, StartNumber, EndNumber
