...
select @reportid = ReportID from Reports where ReportFileName = 'W2GTaxFormRecipients.rpt';

if (@reportid is null)
begin
	insert into Reports (IsActive, ReportFileName, ReportTypeID) values (1, 'W2GTaxFormRecipients.rpt', @reporttypeId);
	select @reportid = ReportID from Reports where ReportFileName = 'W2GTaxFormRecipients.rpt';
end;
...
begin
	insert into ReportLocalizations (ReportID, CultureID, CultureName, ReportDisplayName) values (@reportid, 1033, 'en-US', 'W2G Tax Form Recipients');
end
...
where ReportFileName like 'W2GTaxFormRecipients.rpt';
