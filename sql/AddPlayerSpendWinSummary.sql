...
select @reportid = ReportID from Reports where ReportFileName = 'PlayerSpendWinSummary.rpt';

if (@reportid is null)
begin
	insert into Reports (IsActive, ReportFileName, ReportTypeID) values (1, 'PlayerSpendWinSummary.rpt', @reporttypeId);
	select @reportid = ReportID from Reports where ReportFileName = 'PlayerSpendWinSummary.rpt';
end;
...
begin
	insert into ReportLocalizations (ReportID, CultureID, CultureName, ReportDisplayName) values (@reportid, 1033, 'en-US', 'Player Win/Spend Summary');
end
...
where ReportFileName like 'PlayerSpendWinSummary.rpt';
