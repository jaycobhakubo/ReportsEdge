...
/****** Object:  StoredProcedure [dbo].[spRptPaperInventoryNotInPlay]    Script Date: 01/28/2014 16:08:56 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[spRptPaperInventoryNotInPlay]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[spRptPaperInventoryNotInPlay]
GO
...
-----------------------------------------------------------------------------------------------------------------
-- 2014.01.28 tmp: Copied from spRptInvCenterOnHand, changed to report paper products only that are not in play
-----------------------------------------------------------------------------------------------------------------

CREATE PROCEDURE [dbo].[spRptPaperInventoryNotInPlay]
(	@OperatorID as int
)	
...
WHERE (pdi.OperatorID = @OperatorID OR @OperatorID IS NULL)
	...
	AND ii.iiRetiredDate IS NULL
	And pdi.ProductTypeID = '16'  --Paper
	And ii.iiFirstIssueDate IS NULL
