...
/****** Object:  StoredProcedure [dbo].[spRptSubSalesByProduct]    Script Date: 03/18/2013 10:56:01 ******/
...
-- 03/15/2013: DE10881 - Sales by Product report does not report inventory sales for Pull Tab products with a sale source of Inventory. FIXED
...
--set @OperatorID = 1   
--set @StartDate = '3/15/2013 00:00:00' 
--set @EndDate = '3/15/2013 00:00:00'  
--set @StaffID = 28  
--set @Session = 0  
--->>>>>>>>>>>>>>>>>>>>ENDTEST<<<<<<<<<<<<<<<<  
...
--------------------------------------------  
--**Pulltab**--  //DE10889 knc - (start) : REMOVED Causing duplicate sale on reports
...
(  
 ...
 sessionNbr          int,            -- DE7731  
-- PullTabs money
pullTabs   money,          -- original field, represents paper sales made at a register  
 pullTabSalesFloor  money,          -- DE7731  
 pullTabSalesTotal  money           -- DE7731  
); 
insert @SalesActivity3a  
(  
 productItemName  
    , staffIdNbr, staffName  
    , soldFromMachineId  
...
select   
(sum(PullTabs) + sum( pullTabSalesFloor)) [Pull Tabs] into #pullTab2 
from @SalesActivity3a   

--select (isnull((select [Pull Tabs]  from #PullTabs),0.00) + isnull((select [Pull Tabs] Tab from #pullTab2),0.00)) [Pull Tabs]  into #PullTab3
...
,(select [Pull Tabs]  from #PullTab2) [Pull Tabs]   
...
,#DeviceFees ,#Discount , #Tax , #Voids , #Returns  , #pullTab2 --, #PullTab3  
