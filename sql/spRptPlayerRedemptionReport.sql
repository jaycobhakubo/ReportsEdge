...
/****** Object:  StoredProcedure [dbo].[spRptPlayerRedemptionReport]    Script Date: 10/30/2014 13:30:45 ******/
...
-- 2012.02.24 jkn: DE10173 when redeeming points take into account
--	the quantity redeemed
-- 2014.10.30 tmp: DE12114 Check if the sale succeeded. 
-- =============================================
	@OperatorID	AS	INT,
...
select rr.GamingDate,isnull(sp.GamingSession,0), s.LastName,s.FirstName,rr.TransactionNumber,
       ...
       abs(rd.Quantity * rd.PackagePrice), ((rd.Quantity * rd.TotalPtsRedeemed)*(-1)) as PtsRedeemed --DE10173
from Player p
...
select rr.GamingDate,isnull(sp.GamingSession,0), s.LastName,s.FirstName,rr2.TransactionNumber,
       p.PlayerID,p.LastName,p.FirstName,rd.Quantity,rd.PackageName,  
       (abs(rd.Quantity * rd.PackagePrice ))*(-1),(rd.Quantity * rd.TotalPtsRedeemed) as PtsRedeemed --DE10173
from Player p
	join RegisterReceipt rr on p.PlayerID = rr.PlayerID
	join RegisterDetail rd on rr.RegisterReceiptID = rd.RegisterReceiptID 
	left join TransactionType tt on rr.TransactionTypeID = tt.TransactionTypeID
