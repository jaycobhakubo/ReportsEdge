...
/****** Object:  StoredProcedure [dbo].[sprptUnplayedPacks]    Script Date: 10/07/2013 17:45:33 ******/
...
/******************************************************************************
    ...
    12/02/2011 DE9711: Ensure that RF sales amounts are not doubled when
        transfering a unit then voiding it.
    12/06/2011 DE9582: Added Transaction date to the result set
    12/09/2011 DE9786: Added support for Sale amount included discounts and
        sales tax
    12/13/2011 DE9782: Remove the non-electronic only sales from the report
    12/15/2011 DE9814: When a RF unit is transferred, return the destination
        unit and serial numbers.
    01/17/2012 DE9947: When device fees are charged they are not being returned
        properly
    05/14/2012 DE9788: Serial# on voied pack not showing up. - KC
    2012.06.01 DE9788: Added support for displaying the proper unit numbers
        for sales, voids, and transfers
    2012.06.25 DE9788: Fixed issue with the number of units that a pack was
        loaded into would adjust the the calculation of the voided amount.
    2012.08.03 DE10643 When gathering the void info a pack can be voided 
        before it is entered into a device so this needs to be accounted for
        as well
******************************************************************************/
AS
BEGIN
...
BEGIN  
    ...
    where rr.RegisterReceiptID = @LoopReceiptID 
        ...
        and (ul.ulId = (select max(ulId) from UnlockLog where ulPackNumber = rr.PackNumber)
            or ul.ulId is null) -- DE10643
    group by  
        rr.PackNumber,p.FirstName,p.LastName,sp.GamingSession,rr.DTStamp,rr.DeviceFee
        ,ul.ulUnitNumber,ul.ulUnitSerialNumber,ul.ulSoldToMachineId,m.SerialNumber
