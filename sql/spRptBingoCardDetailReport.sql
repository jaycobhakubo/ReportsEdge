...
/****** Object:  StoredProcedure [dbo].[spRptBingoCardDetailReport]    Script Date: 05/05/2014 16:46:17 ******/
...
-- 2013.01.23 jkn US415/TA65 Removed the limitation for not including overridden
--		sessions.  This was causing issues with reporting on sessions that had
--		sales but were overridden the number of cards sold was not being reported
--		properly.  Now the sessions will be counted and all of the cards sold
--		will displayed on the report.  This was found during the initial NV
--		install at the Sands.
-- 2014.03.26 tmp US3311 Added support for the Card Type 'Double Action'
-- 2014.05.05 tmp DE10321 Fixed issue with Bonus Line numbers not be returned for continuation games.
--=============================================================================
...
insert into @TempCardSales
 ...
 SELECT 
	...
	RDI.CardLvlID, RDI.CardTypeID, CT.CardType, 
	Case when RDI.CardTypeID = 6 Then 3 -- If the card type is 'Double Action' then set the GameTypeID to 'Double Action' so
	 Else RDI.GameTypeID End,           -- that it is displayed as a double action card on the report. 
	RDI.CardLvlName, RDI.CardLvlMultiplier,
...
Where RR.GamingDate >= Cast(Convert(varchar(24),@StartDate,101) as smalldatetime)
	...
	and RDI.CardTypeID in (1, 2, 4, 5, 6)
...
select distinct MasterCardNo, /* DE10321 BonusLineMasterNo */ SessionGamesPlayedID from @TempCardSales where CardTypeId = 2
open bl_cursor
...
begin
    ...
    where /* DE10321 BonusLineMasterNo */ MasterCardNo = @BLMasterNo and SessionGamesPlayedID = @sgpId
