...
/****** Object:  StoredProcedure [dbo].[spRptPaperTransactionDetail_ProductSummary]    Script Date: 10/07/2013 17:50:17 ******/
...
CREATE TABLE #TempTble
	(
		txID int,
		txMID int,
		txGamingDate SmallDateTime,     --DE10584
		txSession int,
		txStaff int,
...
(SELECT txID FROM #TempTble
where 
txGamingDate BETWEEN CAST(CONVERT(varchar(14), @StartDate, 101) AS SMALLDATETIME)      -- DE10584
AND CAST(CONVERT(varchar(14), @EndDate, 101) AS SMALLDATETIME)) b --DE10584
	on a.txMID = b.txID 
...
txValue 
	 ...
	 where 
	 txGamingDate BETWEEN CAST(CONVERT(varchar(14), @StartDate, 101) AS SMALLDATETIME)--DE10584
	AND CAST(CONVERT(varchar(14), @EndDate, 101) AS SMALLDATETIME)--DE10584
	and txMID is null)
