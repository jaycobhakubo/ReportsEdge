-- AddPositionPermissionSummary.sql   
-- US1956
...
select @reportid = ReportID from Reports where ReportFileName = 'PositionPermissionSummary.rpt';

if (@reportid is null)
begin
	insert into Reports (IsActive, ReportFileName, ReportTypeID) values (1, 'PositionPermissionSummary.rpt', @reporttypeId);
	select @reportid = ReportID from Reports where ReportFileName = 'PositionPermissionSummary.rpt';
end;
...
begin
	insert into ReportLocalizations (ReportID, CultureID, CultureName, ReportDisplayName) values (@reportid, 1033, 'en-US', 'Position Permission Summary');
end
...
where ReportFileName like 'PositionPermissionSummary.rpt';
