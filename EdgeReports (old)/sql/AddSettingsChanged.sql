...
select @reportid = ReportID from Reports where ReportFileName = 'SettingsChangedReport.rpt';

if (@reportid is null)
begin
	insert into Reports (IsActive, ReportFileName, ReportTypeID) values (1, 'SettingsChangedReport.rpt', @reporttypeId);
	select @reportid = ReportID from Reports where ReportFileName = 'SettingsChangedReport.rpt';
end;
...
begin
	insert into ReportLocalizations (ReportID, CultureID, CultureName, ReportDisplayName) values (@reportid, 1033, 'en-US', 'System Settings Changed');
end
...
where ReportFileName like 'SettingsChangedReport.rpt';
