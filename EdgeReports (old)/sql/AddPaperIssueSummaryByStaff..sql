-- AddPaperIssueSummaryByStaff.sql   
-- US2081
...
select @reportid = ReportID from Reports where ReportFileName = 'PaperIssueSummaryByStaffReport.rpt';

if (@reportid is null)
begin
	insert into Reports (IsActive, ReportFileName, ReportTypeID) values (1, 'PaperIssueSummaryByStaffReport.rpt', @reporttypeId);
	select @reportid = ReportID from Reports where ReportFileName = 'PaperIssueSummaryByStaffReport.rpt';
end;
...
begin
	insert into ReportLocalizations (ReportID, CultureID, CultureName, ReportDisplayName) values (@reportid, 1033, 'en-US', 'Paper Issue Summary By Staff');
end
...
begin
	...
	insert into ReportDefinitions (ReportID, ReportParameterID) values (@reportid, @p5); -- staffID
end;
...
where ReportFileName like 'PaperIssueSummaryByStaffReport.rpt';
