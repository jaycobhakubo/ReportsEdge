...
/****** Object:  StoredProcedure [dbo].[spRptInvCenterOnHand]    Script Date: 09/07/2012 12:36:47 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[spRptInvCenterOnHand]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[spRptInvCenterOnHand]
GO
...
CREATE PROCEDURE [dbo].[spRptInvCenterOnHand]
(	@OperatorID as int
)	
...
WHERE (pdi.OperatorID = @OperatorID OR @OperatorID IS NULL)
	...
	AND ii.iiRetiredDate IS NULL
	And pdi.ProductTypeID != '17' --Removed Pull Tabs from the report to only report Paper products.
