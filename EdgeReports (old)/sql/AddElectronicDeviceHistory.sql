...
select @p5 = ReportParameterID from ReportParameters where ParameterName = '@SerialNbrDevice';
...
if (@reportid is null)
begin
	insert into Reports (IsActive, ReportFileName, ReportTypeID) values (1, 'ElectronicDeviceHistoryReport.rpt', @reporttypeId);
	select @reportid = ReportID from Reports where ReportFileName = 'ElectronicDeviceHistoryReport.rpt';
end;
...
begin
	insert into ReportLocalizations (ReportID, CultureID, CultureName, ReportDisplayName) values (@reportid, 1033, 'en-US', 'Electronic Device History');
end
...
begin
	...
	insert into ReportDefinitions (ReportID, ReportParameterID) values (@reportid, @p5); -- serialnumber
end;
