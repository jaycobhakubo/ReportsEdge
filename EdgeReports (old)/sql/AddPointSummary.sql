...
select @reportid = ReportID from Reports where ReportFileName = 'PointSummary.rpt';

if (@reportid is null)
begin
	insert into Reports (IsActive, ReportFileName, ReportTypeID) values (1, 'PointSummary.rpt', @reporttypeId);
	select @reportid = ReportID from Reports where ReportFileName = 'PointSummary.rpt';
end;
...
begin
	insert into ReportLocalizations (ReportID, CultureID, CultureName, ReportDisplayName) values (@reportid, 1033, 'en-US', 'Point Summary');
end
...
where ReportFileName like 'PointSummary.rpt';
