...
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'spRptPlayerList') AND type IN (N'P', N'PC')) 
...
-- 2019.05.20 tmp: Removed adding a day to @EndDate parameter 
-- 2019.05.21 tmp: For Hoover Dam, do not include device fees.
-- ============================================================================
 @OperatorID as int,   
...
--set @EndDate = Dateadd(Day,1,@EndDate) 20190520 tmp - Changes made on 04/17/2018 do not require adding a day
			...
			as
				(
					select  ctePS.PlayerID,
							...
							isnull(ctePS.Spend, 0) /*+ isnull(rr.DeviceFee, 0)*/ -- Do not include device fees at Hoover Dam
					from	RegisterReceipt rr
...
and (@State = '' or tpl.State in (select [State] from FnRptPlayerListLocationState(@State)))
	...
	if (@IsPackageName = 1)
	begin
		--set @EndDate = DATEDIFF(day, 1, @EndDate) 20190520 tmp: Don't need to add a day
