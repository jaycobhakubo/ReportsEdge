...
if not exists (select * from Reports where ReportFileName = 'PlayerWinByDate.rpt')
begin

    insert into Reports values (3, 1, 'PlayerWinByDate.rpt');   -- Set Report Group and Set IsActive
    ...
    insert into ReportLocalizations values (@ReturnValue, 1033, 'en-US', 'Player Win by Date');
end