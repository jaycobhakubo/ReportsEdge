...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[spRptGroupedSessionSummaryYTD]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[spRptGroupedSessionSummaryYTD]
GO
...
-- =============================================
-- Author:		FortuNet
-- Description:	<Session Summary Recap Year to Date >
-- US3731 - Add Year to Date to the Session Summary Recap.
...
--=============================================
CREATE PROCEDURE [dbo].[spRptGroupedSessionSummaryYTD]
	@OperatorID as int,
...
begin
	...
	select  DatePart(Year, sp.GamingDate), 
			...
			Isnull((Sum(PaperSales) + Sum(ElectronicSales) + Sum(BingoOtherSales) + Sum(PullTabSales)- Sum(Discounts) /*- couponSales*/ + SUM(ValidationSales) + sum(RedeemFees)), 0),	-- US4521
	...
	from	(SessionSummary SS
			...
			/*(select sum(NetSales) couponSales from dbo.FindCouponSales(@OperatorID,@StartDate,@EndDate,@Session)) as Coupon */
			left join	(	select	GamingDate,
									GamingSession,
									sum(RedeemFees) as RedeemFees
							from	FindPointRedemptions (@OperatorID, @StartDate, @EndDate, @Session) 
							where	Voided = 0
	...
	group by Year(sp.GamingDate)/*,couponSales*/
