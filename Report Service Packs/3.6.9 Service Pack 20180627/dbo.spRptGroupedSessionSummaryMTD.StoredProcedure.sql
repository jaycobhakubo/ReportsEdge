...
-- 20160203 tmp: US4523 Added bank fill to expected cash.
...
begin
	...
	from	(SessionSummary SS
			...
			/*Old school join No need for join*/ /*(select sum(NetSales) couponSales from dbo.FindCouponSales(@OperatorID,@StartDate,@EndDate,@Session)) as Coupon */
			 left join (	select  GamingDate,
						...
						where Voided = 0 
						group by GamingDate, GamingSession) fpr on fpr.GamingDate = sp.GamingDate and fpr.GamingSession = sp.GamingSession	
	...
	group by Year(sp.GamingDate), Month(sp.GamingDate), DATENAME(Month, sp.GamingDate) /*, couponSales*/
	order by Year(sp.GamingDate), Month(sp.GamingDate)
end
else
begin
	Insert into @Results
