...
-- 2018.04.02 tmp: When a parton used points to pay for the device fees the taxable revenue was understated as the
--                 amount was being deducted from revenue as a negative amount in Bingo Other and then again as a device fee.
-- 2018.06.27 tmp: -- 2018.06.01 tmp: Added flag to enable finding the point redemptions. 
-- =============================================
...
begin
	...
	Select	sp.SessionPlayedID,
			...
			sp.GamingSession,
			ss.ManAttendance,
			ISNULL((SUM(ss.PaperSales) + SUM(ss.ElectronicSales) + SUM(ss.BingoOtherSales) - SUM(ss.Discounts) /*+ isnull(cpn.Coupon, 0)*/ + SUM(ss.ValidationSales) + isnull(fpr.RedeemFees, 0)) ,   0) as BingoSales, -- US4522
	...
	Group BY sp.GamingDate, sp.SessionPlayedID, sp.GamingSession, ss.ManAttendance, ss.AccrualPayouts, ss.AccrualIncrease /*, cpn.Coupon*/, fpr.RedeemFees
end
else
begin
	INSERT INTO @Results
	...
	Select	sp.SessionPlayedID,
			...
			ISNULL(
				(SUM(ss.PaperSales) 
				+ SUM(ss.ElectronicSales) 
				+ SUM(ss.BingoOtherSales) 
				- SUM(ss.Discounts) 
				+ SUM(ss.ValidationSales) 
				) ,   0) as BingoSales, 
			ISNULL((SUM(ss.CashPrizes) + SUM(ss.CheckPrizes) + SUM(ss.MerchandisePrizes)), 0) as BingoPrizes,
			ss.AccrualPayouts,
			ss.AccrualIncrease
	...
	Group BY sp.GamingDate, sp.SessionPlayedID, sp.GamingSession, ss.ManAttendance, ss.AccrualPayouts, ss.AccrualIncrease
end
